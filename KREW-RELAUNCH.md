...
1. **Hero** — Ivy creature *film* (glass-liquid macro loop, glowing eyes). ✅ DONE — `components/landing/Hero.tsx`; video `public/hero/ivy-hero.mp4`. Void removal is now **screen-blend + all-edge feather mask** (`EDGE_FEATHER` radial ∩ bottom melt via `mask-composite`): screen resolves the flat black void to page-black, the feather dissolves the hard rectangular frame so no box shows on any bg and the creature floats free (matches the PS comp). Sized **dramatically bigger** — `VIDEO_WIDTH` 216%, `VIDEO_SHIFT`/`VIDEO_DROP` bleed it off the right/bottom. Autoplay muted/loop, poster on reduced-motion. *(Session 2026-07-14 — bigger creature + box removed via edge feather.)*
