...
1. **Hero** — Ivy creature *film* (glass-liquid macro loop, glowing eyes). ✅ DONE — `components/landing/Hero.tsx`; video `public/hero/ivy-hero.mp4`. Void removal is **`mix-blend-mode: lighten`** — `max(video, page)`, so the whole black void *and its H.264 compression noise* fall below page-black and get replaced by it (no box), while the brighter creature + glow survive vibrantly. This replaced `screen`, which LIFTED the void noise into a visible box; also beat a canvas luma-key (which dimmed the glass interior). Relies on the page being dark — fine, marketing is dark-only. Bottom-melt mask into the next section kept; no edge feather needed. Sized **dramatically bigger** — `VIDEO_WIDTH` 240%, `VIDEO_SHIFT`/`VIDEO_DROP` bleed it off the right/bottom. Autoplay muted/loop, poster on reduced-motion. *(Session 2026-07-14→15 — bigger creature + box removed via lighten blend; verified on a real compressed frame.)*
