...
| About/Vision two-list diptych | `dev` | New section after the films (**replaces** the abstract declaration): "Founders should build. Agents should operate." shown, not stated — a before/after pair of **coded** iOS "Today" screens (`components/vision/VisionTodo.tsx`), rebuilt from user PNGs (now `design-refs/vision-todo-{before,after}.png`) so every string lives in COPY.md, crisp/themeable/accessible. On scroll-in the BEFORE screen's 8 grind tasks strike through top→down (staggered), each spawning its **owning agent's mascot** at the row edge (handoff per task; owner = registry slug, not copy); the AFTER screen (founder's calm day, "Make room for what only you can do.") brightens once handled. Motion = scroll-reveal only (§5-safe; films stay the hero moment). Reduced-motion = both resolved static. Data/copy in COPY.md "two-list diptych" + `content/vision-copy.ts` (`VISION_TODO`). Numbers 83/6 are the user's own daily figures, kept verbatim. | **done, 2026-07-16, dev** |
