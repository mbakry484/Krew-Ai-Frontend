...
| About/Vision "Today" list | `dev` | **Rebuild of the diptych** (user rejected the coded-chrome before/after look). Now ONE photographic iPhone frame (`public/vision/iphone-frame.webp` — transparent alpha WebP, status bar baked in, screen bleeds off bottom) with the "Today" list **coded onto the black screen** (`VisionTodo.tsx`). A single day mixes busywork + real work; **scroll-scrubbed** (sticky pin + `useScroll`→`--p`) draws a strike L→R through the `handled` tasks and dims them, leaving the ones that matter. Pure-CSS `calc` off `--p` per row (no per-frame re-render). Describe paragraph (option A, new) sits left of the phone; screen keeps "Today" + "Make room for what only you can do." Screen-inset KNOBS at top of file (overlay measured from the frame: inner screen 18.4–81.2% wide, 21.5% down; can't self-verify live render). Reduced-motion = no pin, `--p`=1 resolved static. Retired: per-task mascots + colored checkboxes. Diptych `VisionTodo` fully replaced. **Fix pass same day:** shipped v1 rendered unstyled — TWO styled-jsx traps, now warned in the file header: (1) JSX factored into a variable gets NO scope class → keep phone markup inline; (2) `${}` interpolation in the style block broke emission → knobs flow as CSS vars via inline style, stylesheet is fully static. Frame asset recropped to the phone bbox (826×1000, lossless alpha WebP). Verified via new **`/dev/vision-todo` harness** + headless Edge screenshots (desktop / reduced-motion / mobile) — NOTE: headless Edge on this machine inflates the CSS viewport ~1.26× (Windows display scale), so narrow-viewport "clipping" in screenshots is rig artifact; trust dump-dom measurements (`docScrollW` vs `winW`). **Size pass:** phone enlarged both views (desktop `min(clamp(340px,40vw,600px),68vh)`, mobile `min(88vw,430px,62vh)`; container 1200); mobile copy top-aligned (rows `auto 1fr`) killing the dead band above it; films-hero mobile bottom padding pb-24→pb-14. Then: mobile phone moved DIRECTLY under the copy (rows `auto auto`, align-content start) — the leftover 1fr space had just moved the gap between text and phone. Final mobile form: user supplied a **complete** phone frame (`iphone-frame-mob.webp`, 643×1359 lossless alpha, real bottom bezel; source `*-src.png` gitignored) — `<picture>` swaps frames at ≤820px, per-frame screen insets via `--scr-t/b` vars (mobile top 10% / bottom 5%). Unpinned mobile was tried and REVERTED — you scrolled away before the strikes drew. Final: mobile pins like desktop (copy top, complete phone under it, lower half viewport-clipped during the pin — reads intentional since the list lives in the top half; all 8 rows verified above the fold). | **done, 2026-07-16, dev** |
| ~~About/Vision two-list diptych~~ | `dev` | *(superseded same day by the "Today" list rebuild above — coded before/after replaced by the photographic single-phone version.)* | superseded |
| About/Vision films-first hero | `dev` | Films promoted to the page hero, ElevenLabs structure: eyebrow + **bold** film-line headline left ("This is what running a brand should look like."), locked vision body right (COPY.md "Films hero"), NO CTA. **Hover expands + plays** on hover-capable devices (pointer over the row pauses auto-advance, resumes on leave; a touch click stops it for good; focus = keyboard hover). Film order relocked: **yoga → dinner → sleep → work**. Declaration ("Founders should build.") is now its own section below the films; its eyebrow moved up to the hero. Same-day v2: hero body relocked benefit-first ("We believe you should run your brand from the front…"); mobile switched from vertical stack to a **horizontal snap slider** (edge-bleed row, hidden scrollbar, snapped card plays, next peeks dimmed; IO threshold 0.6 so two cards never feature at once). | **done, 2026-07-16, dev** |
| About/Vision film rebuild | `dev` | `/about/vision` rebuilt around the four brand films (`public/vision/{sleep,yoga,dinner,work}.mp4`, user-generated): ElevenLabs-style expanding accordion (`components/vision/VisionFilms.tsx`) with coded overlays — editorial center line (fades in ~1.6s after the film breathes) + one agent notification chip (mascot/accent from registry, staggers at ~3.4s, never with the line). Auto-advances ~8s until first click; mobile = vertical stack playing in view; reduced-motion = posters, resolved overlays. Copy locked in COPY.md "ABOUT / VISION" + `content/vision-copy.ts` (yoga chip uses canon 690k/310k, not the drafted 42k). Roster now renders from the registry (was hardcoded Luna=Live/Ivy=Soon — backwards); "The Old Way" / "That era is ending." / stale Mission & Vision retired until the v3 copy pass. Assets: sources gitignored as `*-src.mp4`, shipped H.264 CRF25+faststart (~1.2–3MB) + WebP posters. `next.config.mjs` gained env-gated `NEXT_BUILD_CPUS` throttle for verification builds on memory-pressured machines (deploys unaffected). | **done, 2026-07-15, dev** |
